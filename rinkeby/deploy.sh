...
log "Deploy PreSale"
ethester -v deploy -s $OWNER PreSale \
    @run/Token.addr @run/Beneficiary.addr @run/PrivateSale.addr \
    > run/PreSale.addr
ethester call -q @run/PreSale.addr PreSale.owner       --expect $OWNER
ethester call -q @run/PreSale.addr PreSale.token       --expect @run/Token.addr
ethester call -q @run/PreSale.addr PreSale.beneficiary --expect $OWNER

log "Deploy MainSale"
ethester -v deploy -s $OWNER MainSale \
    @run/Token.addr @run/Beneficiary.addr @run/PreSale.addr \
    > run/MainSale.addr
ethester call -q @run/MainSale.addr MainSale.owner       --expect $OWNER
ethester call -q @run/MainSale.addr MainSale.token       --expect @run/Token.addr
ethester call -q @run/MainSale.addr MainSale.beneficiary --expect $OWNER
...
if [ -n "$BENEFICIARY" ]; then
    ...
    ethester call -q @run/PreSale.addr     PreSale.beneficiary     --expect $BENEFICIARY
    ethester call -q @run/MainSale.addr    MainSale.beneficiary    --expect $BENEFICIARY
else
    BENEFICIARY="$OWNER"
fi

log "Setting mint agent to PrivateSale"
...
if [ -n "$NEW_OWNER" ]; then
    for i in WPTokensBaskets Token Beneficiary \
            PrivateSale PreSale MainSale; do
...
Deployed contracts are:
   ...
   PreSale                `cat run/PreSale.addr`
   MainSale               `cat run/MainSale.addr`
